...
public class Derivative
{
    ...
    public static String deriv(String poly)
    {
        ...
        int pos = 1;
        int front = 0, back = 0;
        //Keeps count of the number of the section. The first sec wont have a symbol in front.
        int count = 0;      
        String derivOut = "";
        String derivSec;
        ...
        {        
            ...
            if(indexPlus == indexMinus) //If there is no more "+" or "-" int the poly.
            {
                break; //Breaks the loop.
            }
            else
            {
                if(indexPlus == -1 || indexMinus == -1) //If there is only one of the 2. 
                {
                    ...
                    else    //If the "-" is the only one left.
                    {
                        ...
                        //Indicates that the next section of the poly is negative. 
                        min2 = true;    
                    }
                }
            ...
            derivSec = poly.substring(front, back); //Section from constant to end of the exponent 
            ...
            if(xpos == -1)      //If there is no x the derivative is 0;
            {
                ...
                count++;    
                min = min2;     //Sets a negative value to the next section of the poly.
            ...
            {
                if(xpos == 0 && exp == 0)   //If there is no constant nor exponent.
                {
                    derivOut += " + 1";
                    min = min2;
                    count++;
            ...
            {
                cost = derivSec.substring(0, xpos);     //Gets the string of the constant
            ...
            {
                String constVal = "";
                if(c - (int)c == 0) //If the constant is an int- converts to int.
                {
                    constVal = "" + (int)c;
                }
        ...
        String cost;
        String section;
        derivSec = poly.substring(front);   //Find the last section after the last '+'
        derivSec = derivSec.trim();         //Trims the section
        xpos = derivSec.indexOf("x");       //Posistion of x
        if(xpos == -1)  //If there is no x in the section
        {
            return derivOut;
        }
        ...
        if(xpos == 0 && exp == 0)       //If there is no constant and no exponent.
        {
            if(count > 0)   //As long as this is not the first section of the poly.
            {
                if(min == true)  //If the section of the poly is negative.
                {
                    section = " - 1";
                }
                else    //If the section of the poly is positive.
                {
                    derivOut = " + 1";
                }
            }
        ...
        if(xpos == 0)   //If no constant is defined.
        {
